...
<div class="container-fluid">
    <div class="card p-4">
        <div class="d-flex justify-content-between mb-3">
            <div>
                <h4 class="fw-bold mb-3">Pending List</h4>
                @isset($email)
            ...
            <div>
                <form action="{{ url('/admin/pending/search/') }}" method="GET">
                    @csrf
                ...
                <form class="mt-3" action="{{ url('/admin/pending/search/date') }}" method="get">
                    @csrf
                    <div class="d-flex">
                        <div>
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="badge-light mb-2"><i class="fas fa-gem mr-2"></i>{{ $user->gem }}</span><span class="text-decoration-underline text-primary ms-1" style="cursor: pointer;" id="gem-{{ $user->id }}">Fill</span>
                            <form class="d-none" id="fill-{{ $user->id }}" action="{{ url('/admin/gem/'.$user->id) }}" method="post">
                                @csrf
                                <div class="mb-2">
                                    <input type="number" class="form-control" name="gem" placeholder="Enter Gems" value="">
                                    <input type="hidden" name="created_by" value="{{ Auth::user()->id }}">
                                </div>
                                <button type="submit" class="btn btn-sm btn-info"><i class="fas fa-gem mr-2"></i>Fill Gems</button>
                            </form>
                        </td>
                        <td>
            ...
            @if (!$users->count())
                <div class="bg-light text-center py-3 my-2">
                    ...
                    <p>Go To <a href="{{ url('/admin/pending-list/') }}" class="btn btn-sm btn-primary">Go To Previous</a></p>
                </div>
            @endif
        </div>
    </div>
...
<script>
    $(document).ready(function () {
        //fill gem on off
        var $gems = $('[id^="gem-"]');
        var $fills = $('[id^="fill-"]').addClass('d-none');

        $gems.on('click', function() {
        var id = this.id.substring(4);
        $("#fill-" + id).toggleClass('d-none');
        });
    });
</script>
@endsection
